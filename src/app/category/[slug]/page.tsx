...
import { BookOpen, Clock, Eye, ChevronLeft, ChevronRight, HelpCircle } from 'lucide-react';
    ...
    <LayoutWrapper>
      <div className="editorial-container py-10 space-y-8 animate-in fade-in duration-300">
        ...
        <div className="bg-orange-50 border border-orange-100 rounded-2xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
          <div>
            <span className="text-[10px] font-bold text-orange-600 uppercase tracking-widest">Category Articles</span>
            <h1 className="text-2xl font-extrabold text-slate-900 mt-0.5">
              {category.name}
            </h1>
            {category.description && (
              <p className="text-slate-500 text-xs mt-1.5 max-w-xl leading-relaxed">{category.description}</p>
            )}
            <p className="text-xs text-orange-600 font-semibold mt-1">Found {total} matched article(s)</p>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            {posts.length === 0 ? (
              <div className="text-center py-24 bg-white border border-dashed rounded-3xl text-slate-400">
                ...
                <p className="text-xs mt-1">We couldn't find any published articles under this category.</p>
              </div>
            ) : (
              <>
